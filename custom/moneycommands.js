var fs = require('fs');
var userdata = fs.readFileSync('custom/moneylog.json');
...
var money = exports.money = {
	...
	giveAmt: function (name, target, target1) {
		...
		fs.writeFile('custom/moneylog.json', finished);
		return;
	},
...
exports.commands = {
	...
	award: function (target, room, user, connection, cmd) {
		...
		if (isNaN(target)) return this.sendReply("That isn't even a number, you egg.");
		if (target < 1) return this.sendReply('You can\'t give ' + targetUser.name + ' anything less than 1 point!');
	...
	atm: function (target, room, user) {
		...
		var format = (money === 1 ? 'Point' : 'Points');

		if (Users.getExact(target)) {
			var line = '<b>' + Users.getExact(target).name + '</b> has ' + moneh + ' ' + format + '.<br/>';
		} else {
			var line1 = '<b>' + target + '</b> has ' + moneh + ' ' + format + '.<br/>';
		}
		this.sendReplyBox(line);
	},

	closeshop: function (target, room, user) {
		if (!this.can('hotpatch')) return false;
		if (shopclosed === true) return this.sendReply('The shop is already closed.');
		shopclosed = true;
	...
	shop: function (target, room, user) {
		...
		this.sendReplyBox('<center><h3><b><u>Point Shop</u></b></h3><table border = "1" cellspacing = "0" cellpadding = "2"><tr><th>Item</th><th>Description</th><th>Price</th><th></th></tr>' +
			...
			'<tr><td>Anim Avatar</td><td>Buys an animated custom avatar. (Costs 20 if you already have a custom avatar)</td><td>40</td><td><button name = "send", value = "/buy animavatar"><b>Buy!</b></button></td></tr>' +
	...
	buy: function (target, room, user) {
		if (shopclosed) return this.sendReply("The shop is closed for now. Wait until it re-opens shortly.");
		target = toId(target);
		...
		if (target === 'symbol') {
			...
			var price = (!Number(user.avatar)) ? 20 : 40;
